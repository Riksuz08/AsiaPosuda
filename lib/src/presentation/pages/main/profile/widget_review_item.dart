...
class ReviewItemList extends StatelessWidget {
  ...
  const ReviewItemList({Key? key, required this.orderModels}) : super(key: key);
    ...
    return SizedBox(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          return WidgetReviewItem(orderModel: orderModels[index]);
        },
      ),
    );
  }
...
class WidgetReviewItem extends StatelessWidget {
  final OrderModel orderModel;
  const WidgetReviewItem({Key? key, required this.orderModel})
      : super(key: key);
...
class WidgetReviewItemInner extends StatelessWidget {
  final LineItems lineItem;
  final OrderModel orderModel;

  const WidgetReviewItemInner({
    Key? key,
  ...
  Future<bool> isEmailFound(HttpService httpService, int productId) async {
            ...
            if (snapshot.connectionState == ConnectionState.waiting) {
              ...
              return FutureBuilder<bool>(
                future: snapshot.data!.parent != 0
                    ? isEmailFound(httpService, snapshot.data!.parent)
                    : isEmailFound(httpService, lineItem.productId),
                builder: (context, emailSnapshot) {
                  if (emailSnapshot.connectionState ==
                    ...
                    return emailSnapshot.data!
                        ? Container() // Don't show container if email is found
                        : Container(
                                    ...
                                    Expanded(
                                      child: Column(
                                        ...
                                        children: [
                                          ...
                                          Text(
                                            'Дата заказа: ${formatTimestamp(orderModel.createdAt)}',
                                            style: TextStyle(
                                                color: Colors.grey,
                                                fontSize: 12),
                                          )
